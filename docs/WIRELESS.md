...
| Shortcuts + LAN upload | Best no-app-code sender candidate | Lets the user send the latest screenshot without modifying their Flutter/RN app. Setup should be generated by PhoneSnap so users never type an IP, port, path, method, or token. |
...
Shortcut setup can be close to instant, but not completely silent. iOS requires the user to approve adding and running a Shortcut. PhoneSnap can still remove all manual configuration:

1. User clicks "Set Up Wireless Shortcut" in the PhoneSnap Mac menu.
2. Mac app starts the local receiver on a stable port.
3. Mac app generates a signed `.shortcut` file with the upload URL and pairing token already baked in.
4. Mac app shows setup options:
   - QR code for `shortcuts://import-shortcut?url=<local install URL>&name=PhoneSnap`
   - AirDrop/share button for the generated `.shortcut`
   - Copy install link fallback
5. User scans or opens the link on iPhone.
6. iOS opens Shortcuts and shows Add Shortcut.
7. First run may ask for Photos/network permission. After that, the user runs "PhoneSnap" from Action Button, Back Tap, Control Center, Home Screen, or Shortcuts.

The generated Shortcut should use a URL like:

```text
http://<mac-hostname>.local:<port>/screenshots?token=<pairing-token>
```

To make this more resilient, generate multiple candidate endpoints into the Shortcut:

```text
http://<mac-hostname>.local:<port>/screenshots?token=<pairing-token>
http://<current-lan-ip>:<port>/screenshots?token=<pairing-token>
```

The Shortcut can try the `.local` hostname first and fall back to the current LAN IP if needed. The user still never sees or types either value.
...
- Shortcut: still the best candidate if the goal is no app-code integration. The setup should be generated locally by PhoneSnap, not hand-authored by the user and not dependent on GitHub/Gist.

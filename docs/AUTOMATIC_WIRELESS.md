...
The app currently relies on Apple exposing the phone through usbmux. A direct Bonjour/TCP fallback remains a possible follow-up; its unfinished experiment was parked outside product sources and was never built or used for capture. This reset test passed capture and app-restart persistence after a prolonged discovery delay whose cause remains unresolved. It does not establish consistently seamless onboarding. A second Mac with no previous pairing would help distinguish a first-use issue from this reset-specific state.

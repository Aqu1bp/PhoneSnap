...
`AutomaticWirelessWatcher` performs photo I/O on one serial worker. Device enumeration selects the saved UDID and explicitly requires Network transport. The same device’s USB presence pauses the wireless connection. `PhoneDeviceConnection` reads the saved pairing HostID via usbmuxd and calls StartSession; it never invokes Pair or Unpair. Only an explicit setup action on a selected USB phone sets and reads back `EnableWifiConnections`.

`DirectPhoneDiscovery` independently browses `_apple-mobdev2._tcp` on main and publishes locked endpoint snapshots. Modern advertisement tags are matched against the selected pairing's HostID-derived key; legacy names use its saved Wi-Fi MAC. If Apple's Network entry is absent or fails to open, `DirectPhoneConnection` opens lockdown TCP port 62078 with the saved pairing. OpenSSL authenticates the host and pins the saved device certificate's public key; the selected device ID is checked inside TLS. AFC uses the returned service port and the same TLS identity when service SSL is requested. `PhonePairingRecord` keeps certificate material in memory. No direct-route operation pairs, writes settings, or changes phone files.

Both transports implement `PhonePhotoConnection` and share catalog/image validation. `PhoneTCP` provides nonblocking, cancellable socket/TLS I/O with monotonic deadlines; each plist or AFC frame shares one deadline across its fragments. Failed streams are discarded. Discovery changes wake the worker immediately; current device ID and generation are captured atomically, and late service callbacks are ignored after stop.
